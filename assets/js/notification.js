const ConversoNotification = (function() {
    ...
    function show(type, message, title = '') {
        const container = document.getElementById('converso-toast-container');
        if (!container) return;

        const toast = document.createElement('div');
        toast.className = `converso-toast ${type}`;
        
        toast.innerHTML = `
            <div class="converso-toast-icon">
                ${createIcon(type)}
            </div>
            <div class="converso-toast-content">
                ${title ? `<div class="converso-toast-title">${title}</div>` : ''}
                <div class="converso-toast-message">${message}</div>
            </div>
            <button class="converso-toast-close" onclick="this.parentElement.remove()">
